...
def read_config(wb) -> dict[str, str]:
    """Read filter values from the Config sheet.

    Expected layout (key in col A, value in col B, starting row 2):
        created_by   | ee42009d-...
        role         | TEACHER
        difficulty   | EASY
    """
    if CONFIG_SHEET not in wb.sheetnames:
        sys.exit(
            f"'{CONFIG_SHEET}' sheet not found in the Excel file.\n"
            "Run with --create-template to regenerate the template with a Config sheet."
        )
    ws  = wb[CONFIG_SHEET]
    cfg: dict[str, str] = {}
    for row in ws.iter_rows(min_row=2, values_only=True):
...
def generate_sql(excel_path: str) -> str | None:
    ...
    for sheet_name, key_index_pairs in MODULE_KEYS.items():
        ...
        for content_key, rows in updates.items():
            ...
            cte_sql, select_cols = build_cte_pair(
                sheet_name, content_key, json_idx, rows, prefix, cfg
            )
...
def create_template(out_path: str = "update_template.xlsx") -> None:
    ...
    ws_cfg.column_dimensions["C"].width = 40
